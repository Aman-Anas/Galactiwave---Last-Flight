...
def runShader(cont):
    ...
    target = own.parent
    radius = own.parent.worldScale #+ own.worldScale
    ownPos = own.worldPosition
    objpos = obj.position
    camerapos = (camera.worldPosition - objpos) / radius[0] * obj.worldOrientation
    lightpos = lamp.getAxisVect([0.0, 0.0, 1.0]) #lamp.getAxisVect([0.0,0.0,1.0])
    lightpos.normalize()
    lightpos = lightpos * obj.worldOrientation

    cameraDistance = camerapos.length# own.parent.scaling[0]
    lightDistance = 1.0#* own.parent.scaling[0]
    m_fOuterRadius = 10.25 #* own.parent.scaling[0]
    m_fInnerRadius = 10.00# * own.parent.scaling[0]
    ...
    own["r"] = radius[0]#* own.parent.scaling[0]
    own["d"] = cameraDistance#own.parent.scaling[0]# #/ own.parent.scaling[0]
           # print(own["d"])
